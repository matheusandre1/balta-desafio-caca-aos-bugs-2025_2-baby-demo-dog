...
public class OrderMethods
{
    ...
    public static async Task<OrderDto> BuildDtoAsync(Order order, IRepository<Product> productRepository, IMapper mapper)
    {
        ...
        var customerDto = mapper.Map<CustomerDto>(order.Customer);

        return new OrderDto(order.Id, order.CustomerId, customerDto!, order.CreatedAt, order.UpdateAt, linesDto);
    }
}
